...
class SI_Widget_Social_Links extends WP_Widget
{
    ...
    {
        parent::__construct('si_widget_social_links', 'Sportland - Социальные сети', [
            'name' => 'Sportland - Социальные сети',
    ...
    public function form($instance)
    {
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('id-link') ?>">Ссылка на соцсеть</label>
            <input
                    type="text"
                    class="widefat"
                    id="<?php echo $this->get_field_id('id-link'); ?>"
                    name="<?php echo $this->get_field_name('id-link'); ?>"
                    value="<?php echo $instance['id-link']; ?>"
            >
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('id-slug') ?>">Выберите соцсеть</label>
            <select name="<?php echo $this->get_field_name('id-slug'); ?>"
                    id="<?php echo $this->get_field_name('id-slug'); ?>">
                <?php foreach ($this->social as $slug=>$desk): ?>
                    <option <?php selected($instance['id-slug'],$slug, true);?> value="<?php echo $slug ?>">
                        <?php echo $desk[0]; ?>
                    </option>
                <?php endforeach; ?>
            </select>
    ...
    public function widget($args, $instance)
    {
        ...
        $link = $instance['id-link'];
        $text = $this->social[$slug][0];
        $svg = $this->social[$slug][1];
?>
